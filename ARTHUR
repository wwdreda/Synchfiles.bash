...
calculate_directory_sha() {
    find "$1" -type f -exec sha256sum {} + | awk '{ print $1 }' | sort | sha256sum | awk '{ print $1 }'
}
...
syncFile(){
    ...
    if [[ "$shasumA" == "$shasumB" ]]; then
        ...
        #keep the file with more recent date or with the bigger size
            ...
            elif (( $(echo "$timeB < $timeA"| bc -l) )); then
                echo "la date A est plus recente"
                cp -d "$fichierA" "$fichierB"
            else
...
# Chemins vers les répertoires A et B
dirA=$1
dirB=$2

# Calculer les sommes de contrôle SHA initiales des répertoires A et B
initialShaA=$(calculate_directory_sha "$dirA")
initialShaB=$(calculate_directory_sha "$dirB")

# Appeler la fonction de synchronisation
synchroniser "$pathA" "$pathB" "."
syncJournal

# Calculer les sommes de contrôle SHA après la synchronisation
finalShaA=$(calculate_directory_sha "$dirA")
finalShaB=$(calculate_directory_sha "$dirB")

# Comparer les sommes de contrôle initiales et finales pour détecter les changements
if [[ "$initialShaA" != "$finalShaA" ]]; then
    echo "Le répertoire A a été modifié pendant la synchronisation !"
    # Gérer la situation critique ici
fi

if [[ "$initialShaB" != "$finalShaB" ]]; then
    echo "Le répertoire B a été modifié pendant la synchronisation !"
    # Gérer la situation critique ici
fi
